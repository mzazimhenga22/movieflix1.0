...
Future<String> uploadMedia(dynamic mediaFile, String type) async {
  try {
    ...
    if (kIsWeb) {
      if (mediaFile is html.File) {
        ...
        if (type == 'photo') {
          ...
          if (bytes.lengthInBytes > 10 * 1024 * 1024 * 1024) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        ...
        debugPrint('Invalid file type for the web platform');
        return 'https://via.placeholder.com/150';
      }
    } else {
  ...
  Future<void> _postStory() async {
    ...
    if (choice != null && mounted) {
      ...
      if (pickedFile != null) {
        ...
        try {
          final uploadedUrl = await uploadMedia(pickedFile, choice);
          if (!mounted) return;
