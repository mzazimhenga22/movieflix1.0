...
// Added dependencies for encryption
...
class SocialReactionsScreenState extends State<SocialReactionsScreen>
  ...
  // **New Method**: Generates an AES encrypter based on a unique ID (e.g., post ID or conversation ID)
  encrypt.Encrypter _getEncrypter(String id) {
    final keyBytes = sha256.convert(utf8.encode(id)).bytes;
    final encryptionKey = encrypt.Key(Uint8List.fromList(keyBytes));
    return encrypt.Encrypter(encrypt.AES(encryptionKey));
  }

  // **New Method**: Decrypts a message if it’s encrypted
  String _decryptMessage(String encryptedText, String id, String? ivBase64) {
    if (ivBase64 == null || encryptedText.isEmpty) return encryptedText;
    try {
      final iv = encrypt.IV.fromBase64(ivBase64);
      final encrypter = _getEncrypter(id);
      return encrypter.decrypt64(encryptedText, iv: iv);
    } catch (e) {
      debugPrint('Error decrypting message for ID $id: $e');
          ...
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              final posts = snapshot.data!.docs.map((doc) {
                ...
                // Assume 'post' might be encrypted; check for 'iv' field
                String message = (data['post'] as String?) ?? '';
                if (data.containsKey('iv') && data['iv'] != null) {
                  message =
                      _decryptMessage(message, doc.id, data['iv'] as String?);
                }
                return {
                  'id': doc.id,
                  ...
                  'iv': data['iv']
                      as String?, // Initialization vector for decryption
                };
              }).toList();
              ...
              return StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  final comments = snapshot.data!.docs.map((doc) {
                    ...
                    if (data.containsKey('iv') && data['iv'] != null) {
                      commentText = _decryptMessage(
                          commentText, doc.id, data['iv'] as String?);
                    }
                    return {
                      ...data,
                      'text': commentText,
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            if (controller.text.isNotEmpty) {
                              try {
                                // Encrypt the comment before posting
                                final iv = encrypt.IV.fromLength(16);
                                final encrypter = _getEncrypter(post['id']);
                                final encryptedText =
                                    encrypter.encrypt(controller.text, iv: iv);
                                await FirebaseFirestore.instance
                                    .collection('users')
                                    .doc(post['userId'])
                                    .collection('posts')
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => IndividualChatScreen(
          ...
          // Assuming storyInteractions might be needed; adjust as per actual implementation
          storyInteractions: [],
        ),
      ),
    );
